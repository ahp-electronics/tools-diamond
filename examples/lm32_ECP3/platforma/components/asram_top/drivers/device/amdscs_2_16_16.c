/****************************************************************************
**
**  Name:  AmdSCS_2_16_16.c
**
**  Description:
**     Performs all of the erase and programming functions for a
**     SPANSION CFI FLASH in a 2 x 16-bit configuration
**
**  $Revision: $
**
** Disclaimer:
 ...
 ****************************/
   ...
   static FlashBoardCfgInfo_t AmdSCS_2_16_16Info;
   static FlashCfgFnTbl_t AmdSCS_2_16_16FnTbl;
   FlashConfiguration_st AmdSCS_2_16_16;
 ...
 ****************************/
   ...
   static void AmdSCS_2_16_16_Init(CFIFlashDevCtx_t *ctx);
   static unsigned int AmdSCS_2_16_16_Reset(CFIFlashDevCtx_t *ctx);
   static unsigned int AmdSCS_2_16_16_ProgramData(CFIFlashDevCtx_t *ctx, unsigned int ByteOffset, unsigned char *Data, unsigned int Bytes);
   static unsigned int AmdSCS_2_16_16_SectorInfo(CFIFlashDevCtx_t *, unsigned int ByteOffset, unsigned int *SAddr, unsigned int *ByteLen);
   static unsigned int AmdSCS_2_16_16_EraseRegion(CFIFlashDevCtx_t *ctx);
   static unsigned int AmdSCS_2_16_16_EraseSector(CFIFlashDevCtx_t *ctx, unsigned int Offset);
   static unsigned int AmdSCS_2_16_16_Write32(CFIFlashDevCtx_t *ctx, unsigned int Offset, unsigned int uiData);
   static unsigned int AmdSCS_2_16_16_WriteData(CFIFlashDevCtx_t *ctx, unsigned int ByteOffset, unsigned char *data, unsigned int Bytes);
...
unsigned int Register_AmdSCS_2_16_16_Configuration(void)
{
    ...
    *----------------------------------------------
    */
   AmdSCS_2_16_16Info.parallels = 2;
   AmdSCS_2_16_16Info.mode = 16;
    ...
    *----------------------------------------------
    */
   AmdSCS_2_16_16FnTbl.Init          = AmdSCS_2_16_16_Init;
   AmdSCS_2_16_16FnTbl.WriteData     = AmdSCS_2_16_16_WriteData;
   AmdSCS_2_16_16FnTbl.WriteData8    = 0;
   AmdSCS_2_16_16FnTbl.WriteData16   = 0;
   AmdSCS_2_16_16FnTbl.WriteData32   = AmdSCS_2_16_16_Write32;
   AmdSCS_2_16_16FnTbl.EraseChip     = AmdSCS_2_16_16_EraseRegion;
   AmdSCS_2_16_16FnTbl.EraseSector   = AmdSCS_2_16_16_EraseSector;
   AmdSCS_2_16_16FnTbl.Reset         = AmdSCS_2_16_16_Reset;
   AmdSCS_2_16_16FnTbl.ProgramData   = AmdSCS_2_16_16_ProgramData;
   AmdSCS_2_16_16FnTbl.SectorInfo    = AmdSCS_2_16_16_SectorInfo;
    ...
    *----------------------------------------------
    */
   AmdSCS_2_16_16.VendorCSId =  0x2;    /* AMD/Fujitsu Standard Command Set */
   AmdSCS_2_16_16.boardInfo  =  &AmdSCS_2_16_16Info;
   AmdSCS_2_16_16.cfgFnTbl   =  &AmdSCS_2_16_16FnTbl;
...
static unsigned int AmdSCS_2_16_16_Reset(CFIFlashDevCtx_t *ctx)
{
   ...
   pt_CFIFlashDev->base = CFI_2_16_FF;
   return(0);
}
 ...
 */
static unsigned int
AmdSCS_2_16_16_EraseRegion(CFIFlashDevCtx_t *ctx)
{
   /* 
   ...
   pDWA_FlashDev->Addr555 = CFI_2_16_AA;
   pDWA_FlashDev->Addr2AA = CFI_2_16_55;
   pDWA_FlashDev->Addr555 = CFI_2_16_80;
   pDWA_FlashDev->Addr555 = CFI_2_16_AA;
   pDWA_FlashDev->Addr2AA = CFI_2_16_55;
   pDWA_FlashDev->Addr555 = CFI_2_16_10;
   ...
   do{
      ...
      if(pDWA_FlashDev->base == CFI_2_16_ERASED)
         return(0);
 ...
 */
static unsigned int
AmdSCS_2_16_16_EraseSector(CFIFlashDevCtx_t *ctx, unsigned int ByteOffset)
{
   pt_DWORD_ALIGNED_CFI_FLASH pDWA_FlashDev = (pt_DWORD_ALIGNED_CFI_FLASH)ctx->base;
   unsigned int *writePtr = (unsigned int *)(ctx->base + ByteOffset);
   unsigned int maxTOMilliSecs;

   /* make sure byteoffset is aligned on a dword boundary */
   if(ByteOffset & 0x3)
 ...
 */
static unsigned int
AmdSCS_2_16_16_WriteData(CFIFlashDevCtx_t *ctx, unsigned int ByteOffset, unsigned char *data, unsigned int Bytes)
{
   unsigned int *puiData;
   unsigned int loopVar;

   /* make sure bytes to write is a multiple of 4 */
   if((Bytes & 0x3) != 0)
      return(MICOFLASH_ERR_DATA_MULTIPLE);

   /* make sure byte-offset is aligned on a 4-byte boundary for this configuration */
   if((ByteOffset & 0x3) != 0)
      return(MICOFLASH_ERR_OFFSET_ALIGN);
   ...
   /* we're all okay to perform writes : use the 32-bit write function to accomplish this */
   puiData = (unsigned int *)data;
   for(loopVar = ByteOffset; loopVar < (ByteOffset + Bytes); loopVar+=4){
      if(AmdSCS_2_16_16_Write32(ctx, loopVar, *puiData)!= 0)
...
/*
 * Program a dword of data
 */
static unsigned int
AmdSCS_2_16_16_Write32(CFIFlashDevCtx_t *ctx, unsigned int ByteOffset, unsigned int uiData)
{
   pt_DWORD_ALIGNED_CFI_FLASH pDWA_FlashDev = (pt_DWORD_ALIGNED_CFI_FLASH)ctx->base;
   unsigned int *writePtr = (unsigned int *)(ctx->base + ByteOffset);
   ...
   /*
    * Fetch maximum timeout for writing a word of data;
    * if none is provided, default to 1 millisecond
    */
   ...
   pDWA_FlashDev->Addr555 = CFI_2_16_AA;
   pDWA_FlashDev->Addr2AA = CFI_2_16_55;
   pDWA_FlashDev->Addr555 = CFI_2_16_A0;
...
static unsigned int AmdSCS_2_16_16_ProgramData(CFIFlashDevCtx_t *ctx, unsigned int ByteOffset, unsigned char *Data, unsigned int Bytes)
{
   ...
   sAddr = ByteOffset;
   do{
      retVal = AmdSCS_2_16_16_SectorInfo(ctx, sAddr, &sAddr, &byteLen);
      if(retVal != 0)
         return(retVal);
      AmdSCS_2_16_16_EraseSector(ctx, sAddr);
      sAddr += byteLen;
   ...
   retVal = AmdSCS_2_16_16_WriteData(ctx, ByteOffset, Data, Bytes);
...
/* Fetch Sector Info */
static unsigned int AmdSCS_2_16_16_SectorInfo(CFIFlashDevCtx_t *ctx, unsigned int ByteOffset, unsigned int *pSAddr, unsigned int *pByteLen)
{
   int temp;
   ...
   /* align address on a dword boundary */
   ByteOffset &= (~0x3);
   ...
   for(loopVar1 = 0; loopVar1 <  ctx->CFIInfo.geometry.totalEraseRegions; loopVar1++){
      for(loopVar2 = 0; loopVar2 < ctx->CFIInfo.geometry.eraseRegion[loopVar1].eraseBlocks; loopVar2++){
         temp = (ctx->CFIInfo.geometry.eraseRegion[loopVar1].blockSize*2);
         eAddr += temp;
         if(ByteOffset < eAddr){
            *pSAddr = sAddr;
            *pByteLen = temp;
...
static void AmdSCS_2_16_16_Init(CFIFlashDevCtx_t *ctx)
{
   ...
   /* interrogate CFI-PRI table to figure out if the device is a top-boot/bottom-boot device */
   IssueCFIQueryEnter(ctx->base, &(ctx->CFIInfo.boardCfg));

   MicoFlushDataCache();
   ucBoot = CFI_READ_BYTE(4, (ctx->base + 
                              (CFI_WP_PROTECTION*sizeof(int))));
   ...
   pDWA_FlashDev->Addr555 = CFI_2_16_AA;
   pDWA_FlashDev->Addr2AA = CFI_2_16_55;
   pDWA_FlashDev->Addr555 = CFI_2_16_90;
    ...
    */
   if(pDWA_FlashDev->base == CFI_2_16_MXID){
      /* IF IT's A MX29LV128M DEVICE, USE PRE-DETERMINED DATA */
      if((pDWA_FlashDev->base1 & CFI_2_16_MASK) == CFI_2_16_MXDEV){
         ...
         if((ucBoot == 2) || (ucBoot == 3)){
            ...
            /* it's an H/L part, all sectors are uniform of 64kbytes/sector/device */
   ...
   /* based on whether it's a top/bottom boot, rearrange sectors for AMD-class devices */
   if(ucBoot == 3){
      /* it's a top-boot device */
