...
# $Header: //synplicity/map202003lat/misc/map.tcl#1 $
...
proc args_to_strg {l_args} {
        set q_exp  {((^\[\s*(get_cells|get_clocks|get_nets|get_pins|get_ports|get_keepers|all_clocks|all_inputs|all_outputs|all_registers|all_keepers|get_flat_cells|get_flat_pins|get_flat_nets))|(^-)){1}}
        set q_exp1 {(^(get_cells|get_clocks|get_nets|get_pins|get_ports|get_keepers|all_clocks|all_inputs|all_outputs|all_registers|all_keepers|get_flat_cells|get_flat_pins|get_flat_nets)$){1}}
        set str ""
        foreach x $l_args {
          if {[regexp $q_exp $x]} {
            set str "$str $x"
...
proc get_flat_cells { args } {
    ...
    if {$_lib_ == "SYNPLIFY"} {
        ...
        set val [eval synplify_get_flat_cells $args]
        if {$fdc_query} {
          print_fdc_query "get_flat_cells" $args $val
        }
	return $val
    } else {
        set args [args_to_strg $args]
        return "\[get_flat_cells $args\]"
    }
...
proc get_flat_nets { args } {
        ...
        if {$_lib_ == "SYNPLIFY"} {
          ...
          set val [eval synplify_get_flat_nets $args]
          if {$fdc_query} {
            print_fdc_query "get_flat_nets" $args $val
          }
          return $val
        } else {
          set args [args_to_strg $args]
          return "\[get_flat_nets $args\]"
        }
}
...
proc ldc_get_groups { args } {
	return "\[ldc_get_groups $args\]"
}
proc get_flat_pins { args } {
    ...
    if {$_lib_ == "SYNPLIFY"} {
          ...
          set val [eval synplify_get_flat_pins $args]
          if {$fdc_query} {
            print_fdc_query "get_flat_pins" $args $val
          }
          return $val
    } else {
        set args [args_to_strg $args]
        return "\[get_flat_pins $args\]"
    }
}
...
#
# Lattice specific attribute mechanism
#
# args: [-disable] object propname str
#
	
proc ldc_create_group {args} {

	addConstraint2Database [lindex [info level 0] 0] [lrange [info level 0] 1 end]
}
proc ldc_create_region {args} {

	addConstraint2Database [lindex [info level 0] 0] [lrange [info level 0] 1 end]
}
proc ldc_create_vref {args} {

	addConstraint2Database [lindex [info level 0] 0] [lrange [info level 0] 1 end]
}

proc ldc_set_location {args} {
	addConstraint2Database [lindex [info level 0] 0] [lrange [info level 0] 1 end]
}

proc ldc_set_vcc {args} {

	addConstraint2Database [lindex [info level 0] 0] [lrange [info level 0] 1 end]
}
proc ldc_set_attribute {args} {

	addConstraint2Database [lindex [info level 0] 0] [lrange [info level 0] 1 end]
}
proc ldc_set_sysconfig {args} {

	addConstraint2Database [lindex [info level 0] 0] [lrange [info level 0] 1 end]
}
proc ldc_set_port {args} {
	
	addConstraint2Database [lindex [info level 0] 0] [lrange [info level 0] 1 end]
}
proc ldc_prohibit {args} {
