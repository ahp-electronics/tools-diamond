	...
	proc xdc2fdc_one_file {xdc_in_file fdc_out_file} {
		...
		if {![catch {open $xdc_in_file r} fpi]} { 
			while {![eof $fpi]} {
				...
				regsub -all {\-no_traverse} $printable_long_line "" printable_long_line
				if {![regexp {^\s*#} $printable_long_line] && ![regexp {\-filter .*NAME} $printable_long_line]} {
					regsub {\"([^\"]+)\"(\s*)} $printable_long_line {{\1}\2} printable_long_line
				}
				...
				if {[regexp {define_current_instance} $printable_long_line]} {
					if {![regexp {\-top} $printable_long_line]} {
						...
						if {[regsub -all {\{|\}} $printable_long_line_new "" printable_long_line_new]} {
							regsub {(\$.*)} $printable_long_line_new {"\1"} printable_long_line_new
							regsub -all {\\\.} $printable_long_line_new {\\\.} printable_long_line_new
							regsub -all {\[|\]} $printable_long_line_new {\\\\&} printable_long_line_new
						}
...
#		regsub -all {([\s\{]+)([^\s=!]+)([=!~]+)([^\s\]\}]+)} $printable_long_line {\1{\2 \3 \4}} printable_long_line
		...
		if {!$skip_clean} {
			set printable_long_line [clean_object $printable_long_line]
...
#		regsub -all {get_clocks} $printable_long_line {get_clocks -nocase} printable_long_line
	...
	proc clean_object {line} {
		...
		while {1} {
### Inner loop for nested [get_...]; if nested, we know  there is only one pattern, and it is in the inner most ###
			while {1} {
				set curr_obj [get_bracketed_sqr $obj]
				if {$curr_obj eq "<>"} {
					break
				}
				set none 0
				if {[regexp {\[\s*get_.*(\[\s*get_.*)} $curr_obj obj_1 obj_2]} {
					set obj $obj_2
					set nest 1
					incr inx
					continue
			...
			incr inx
			if {$inx > 80} {break; puts "ABORT: $curr_obj"}
			if {[regexp {(\[\s*get_.*)(\[\s*list\s+.*)} $curr_obj obj_1 obj_2 obj_3] && ![regexp {\{|\}} $curr_obj]} {
				set obj_3 [get_bracketed_sqr $obj_3]
				regsub {(\[list\s+)(.*)\s*(\]$)} $obj_3 {\1{\2}\3} curr_obj_new
				regsub -all {\s+} [get_bracketed_crly $curr_obj_new] "\} \{" tmp
				set curr_obj_new [sub_strg [get_bracketed_crly $curr_obj_new] $tmp $curr_obj_new]
				set line [sub_strg $curr_obj ${obj_2}$curr_obj_new\] $line]
				set is_list 1
			}
			if {![regexp {\{|\}} $curr_obj] && !$is_list} {
				regsub {\s+([^-\s]+)\s*(\]$)} $curr_obj { {\1}\2} curr_obj_new
				set line [sub_strg $curr_obj $curr_obj_new $line]
			}
			if {!$nest} {
				set obj [sub_strg $curr_obj "" $inc_line]
			} else {
				set obj [sub_strg [get_bracketed_sqr $inc_line] "" $inc_line]
			}
			set inc_line $obj
			set none 1
			set nest 0
...
#### The next command can crash if the $old expression is too big due to an extremly long line  ####
#### Will abort in this case  ####
          if {[catch {regsub $old $strg $new strg} didit]} {
            return "<>"
          }
