...
#            set line [all_in_out $line]
            set line [clean_line_sdc $line]
            regsub -all {\s(\$[^ ]+)} $line { {\1}} line
            set line [::xilinx_ip_constraints::clean_object $line]
            switch -regexp -- $line {
              {create_clock} {puts $fpo_fdc_clock $line}
...
if {[product_type] eq "protocompiler" || [product_type] eq "protocompiler_dx" || [product_type] eq "protocompiler_s" || [product_type] eq "protocompiler100"} {
...
#puts "CMD_final:  $cmd"
  if {!$xdc_syn} {
    ...
    set stat_now 1
    while {$stat != $stat_now} {
      set stat_now [eval $cmd]
      if {$stat == $stat_now || $stat_now == 0} {
        break
      }
      set stat $stat_now
    }
    set supp_err 0
    if {![regexp {load_sta} $cmd]} {
      set stat [eval $cmd]
    }
  }
  cd $start_dir
