...
`uvm_field macros - Instead of implementing the ~do_*~ methods directly,
you may opt to employ the `uvm_field macros, which expand into code that
...
For details on these and other costs associated with `uvm_field and all
other UVM macros, see the white paper, ~OVM/UVM Macros: A Cost-Benefit Analysis~,
...
See <Quickstart 1-2-3 to running examples> for setup requirements for
