#===========================================================================
# @(#) $Id: xlcheck.pl 1869 2019-09-16 23:14:03Z jstickle $
#===========================================================================
...
@toolVersions = (
    "questa 10.6* 10.7*",
    "gcc 4.5.0 4.7.4 4.9.1 5.3.0",
    "osciSysc 2.2.* 2.3.*",
    "vista 4.2.0 4.2.1 4.4.0",
    "ius 13* 14*" );
...
sub checkGcc() {
    if( $toolsToCheck{"env_gcc"} eq "yes" ) {
        ...
        my $gccBanner = `gcc --version 2>&1 | head -1`;
    ...
    if( $toolsToCheck{"env_questa"} eq "yes" ) {
        ...
        if( -r "modelsim.ini" ){
            ...
            if( $iniCppPath =~ /CppPath.*= *(.*)/ ){
                local $cppPath = $1;
                local $iniGccVersion = "";
                local $iniGccBanner = `$cppPath --version 2>&1 | head -1`;
