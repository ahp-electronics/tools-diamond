...
public:
  ...
  // PACK
  void pack_int_n(sc_dt::uint64 k, unsigned nbits);
  void pack_char(char);
  ...
  // UNPACK
  sc_dt::uint64 unpack_int_n(unsigned nbits);
...
void uvmc_packer_rep::pack_int_n(sc_dt::uint64 a, unsigned n) {
  ...
  if (n <= bit_index) {
    ...
    val = *wr_bits_ptr;
    val = (val & (~(((sc_dt::uint64)-1) << bit_index) |
          ~(((sc_dt::uint64)-1)<<(bit_index-n)))) | (a << (bit_index-n));
    *wr_bits_ptr = val;
    ...
    *wr_bits_ptr = val;
    ...
    if (n > 0) {
      ...
      val = ((~(-1<<(bit_index-n))) & val) |
            ((a & (~(((sc_dt::uint64)-1)<<n))) << (bit_index-n));
      *wr_bits_ptr = val;
      update_indexes(n);
    }
  }
...
void uvmc_packer_rep::pack_int_n(sc_dt::uint64 a, unsigned n) {
  ...
  if (n <= (unsigned)(32-bit_index)) {
    ...
    val = *wr_bits_ptr;
    val = (val & (~(((sc_dt::uint64)-1) << bit_index) |
          (((sc_dt::uint64)-1)<<(bit_index+n)))) | (a << (bit_index));
    *wr_bits_ptr = val;
    ...
    val = *wr_bits_ptr;
    val = (val & (~((((sc_dt::uint64)-1) << bit_index)))) |
          ((a & ~(((sc_dt::uint64)-1)<<(32-bit_index))) << bit_index);
    *wr_bits_ptr = val;
    n -= (32-bit_index);
    a = a >> (32-bit_index);
...
void uvmc_packer_rep::pack_double(double a) {
  sc_dt::uint64* uint64_p = reinterpret_cast<sc_dt::uint64*>(&a);
  sc_dt::uint64 val = *uint64_p;
  pack_int_n(val,64);
}

// TODO: account for time unit
void uvmc_packer_rep::pack_sc_time(const sc_time& a) {
  sc_dt::uint64 val = a.value();
  pack_int_n(val,64);
}
...
#ifdef MSB_FIRST
sc_dt::uint64 uvmc_packer_rep::unpack_int_n(unsigned int n) {
  ...
  if (n <= bit_index) {
    val = ( ((~(((sc_dt::uint64)-1) << bit_index)) &
            (((sc_dt::uint64)-1) << (bit_index-n)) ) & val) >> (bit_index-n);
    update_indexes(n);
    return val;
  }
  ...
  sc_dt::uint64 allbits= 0;

  // left-bits
  allbits = ( (~(((sc_dt::uint64)-1) << bit_index)) & val ) << (n-bit_index);
...
sc_dt::uint64 uvmc_packer_rep::unpack_int_n(unsigned int n) {
  ...
  if (n <= (unsigned)(32-bit_index)) {
    val = (val >> bit_index) & (~(((sc_dt::uint64)-1) << n));
    update_indexes(n);
  ...
  sc_dt::uint64 allbits= 0;
  sc_dt::uint64 numbits = 32-bit_index;

  // right-bits
  allbits = (val >> bit_index) & (~(((sc_dt::uint64)-1)<<numbits));
  n -= numbits;
  ...
  if (n > 0) {
    ...
    //allbits = allbits |
    //     (((((sc_dt::uint64)-1)<<(bit_index-n)) & val) >> (bit_index-n));
    allbits = allbits | (((~(((sc_dt::uint64)-1)<<n)) & val) << numbits);
...
void uvmc_packer_rep::unpack_double(double& a) {
  sc_dt::uint64 val = unpack_int_n(64);
...
def_pack_int_operators(sc_dt::int64)
def_pack_int_operators(sc_dt::uint64)
