...
//void  SV2C_blocking_rsp_done(int x_id, const bits_t *bits, sc_dt::uint64 delay);
}
      ...
      public virtual tlm_transport_if<T1,T2>
{
  ...
  virtual void blocking_rsp_done(const bits_t *bits, sc_dt::uint64 delay) {
    //int sz, id;
...
{
  protected:
  // This is made mutable so that const methods (such as ::peek()) can wait
  // on this semaphore even though they do not otherwise alter this object.
  mutable sc_semaphore m_sem;
  ...
  virtual void b_transport( T& trans, sc_time& t )
  {
    ...
    delay_in_ps = (double)m_delay_bits;
                                              ...
                                              sc_core::sc_time& t)
  {
    ...
    m_packer->init_unpack(m_bits);
    CVRT::do_unpack(trans,*m_packer);
    delay_in_ps = (double)(*delay_in_bits);
    t = sc_time(delay_in_ps,SC_PS);
    return sync;
  }
  ...
  virtual int x_nb_transport_bw (bits_t *bits, uint32 *phase, sc_dt::uint64 delay) {
  ...
  */
  ...
  virtual int x_nb_transport_bw (bits_t *bits,
    ...
    *phase = x_phase;
    ...
    delay = reinterpret_cast<sc_dt::uint64*>(&delay_in_ps);
