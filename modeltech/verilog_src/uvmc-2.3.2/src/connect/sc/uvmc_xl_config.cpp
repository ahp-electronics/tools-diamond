...
{   assert(w <= 32);
    ...
    if( iMod32 == 0 && w == 32 ) // Efficient way of processing
        return s[i>>5];          // for case of aligned 32 bit slices
...
{   assert(w <= 32);
    ...
    unsigned iMod32 = i & 0x1f;
    if( iMod32 == 0 && w == 32 ){ // Efficient way of processing
        d[i>>5] = s;              // for case of aligned 32 bit slices
        return;
    }
    unsigned iPlusRange = iMod32 + w;
    unsigned loMask = 0xffffffff << iMod32;
...
{   unsigned short i;
    ...
    if( static_config_num_bytes ) {
        m_is_static_config_deletion_enabled = true;
    ...
    if( sideband_config_num_bytes ) {
        m_is_sideband_config_deletion_enabled = true;
