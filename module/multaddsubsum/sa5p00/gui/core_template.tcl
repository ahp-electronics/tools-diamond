#$Header: L:/module/multaddsubsum/sa5p00/gui/core_template.tcl 1.1 mghuang Exp $

#Rev 1.1: Diamond 2.2, IPExpress: SCR_1021
...
proc ChangeValueCmd {} {
	...
	switch $Para(addsub0) {
	    "Add" {
			switch $Para(addsub1)  {
				"Add" {.lblEquation config -text "SUM = ((A0*B0 + A1*B1) + (A2*B2 + A3*B3))"}
				"Sub"   {.lblEquation config -text "SUM = ((A0*B0 + A1*B1) + (A2*B2 - A3*B3))"}
				"Dynamic"  {.lblEquation config -text "SUM = ((A0*B0 + A1*B1) + (A2*B2 +/- A3*B3))"}
			} 
		}
	    "Sub"   {
			switch $Para(addsub1)  {
...
proc Create_Image {} {
	...
	switch $tcl_platform(platform) {
		windows {AttributeSetting 500 590}
		unix    {AttributeSetting 530 690}
	}
	set pin_flag 1
	DrawImage 260 450
	set tnb $Para(tnb)
