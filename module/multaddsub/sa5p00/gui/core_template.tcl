#$Header: L:/module/multaddsub/sa5p00/gui/core_template.tcl 1.1 mghuang Exp $
...
proc ChangeValueCmd {} {
	...
	if {$Para(addsub) == "Add"} {
		.lblEquation config -text "SUM = (A0*B0 + A1*B1)"
	}
	if {$Para(addsub) == "Sub"} {
		.lblEquation config -text "SUM = (A0*B0 - A1*B1)"
	}
...
proc Create_Image {} {
	...
	switch $tcl_platform(platform) {
		windows {AttributeSetting 500 560}
		unix    {AttributeSetting 550 650}
	}
	set pin_flag 1
	DrawImage 260 450
	set tnb $Para(tnb)
