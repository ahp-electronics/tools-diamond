#$Header: L:/module/fifo/gui/core_template.tcl 1.19 mghuang Exp $

#Rev 1.19: Diamond 2.0 IPExpress: Fixed CR_54370
#Rev 1.18: Diamond 1.4 ECP4 IPExpress: Fixed cr_54612
#Rev 1.17: Diamond 1.2 IPExpress: ECP4 support.
...
proc ChangeECP4ValueCmd {} {
	global Para
	
	#CR_54370
	if {$Para(FIFOImp) == "LUT Based"} {
		.ckEnFWFT config -state disable
		set Para(EnFWFT) 0		
	} else {
		.ckEnFWFT config -state normal
	}
}
...
proc Mg5g00CmdLine {}  {
	...
	set str "-arch $Para(arch) -type fifoblk"
...
proc Or5s00CmdLine {} {
	global Para
	set str "-arch $Para(arch) -type ebfifo -sync_mode -depth $Para(Depth) -width $Para(Width)" ;#-rwidth $Para(RWidth)
	if {$Para(FIFOImp) == "LUT Based"} {lappend str -pfu_fifo}
...
proc Create_Image {} {
	...
	frame .frmType
	radiobutton .rdoEBR -text "EBR Based" -width 10 -value "EBR Based" -variable Para(FIFOImp) -anchor w -command ChangeValueCmd
	radiobutton .rdoLUT -text "LUT Based" -width 10 -value "LUT Based" -variable Para(FIFOImp) -anchor w -padx 100 -command ChangeValueCmd
	pack .rdoEBR .rdoLUT -in .frmType -side left -padx 10

	frame .frmAdd_Data
	frame .frmAddress
	iwidgets::entryfield .entAddress -labeltext "Address_depth" -labelpos w -textvariable Para(Depth) \
		-width 8 -validate numeric -fixed 6
	label .lblAddress -text "(2-$MaxAddDepth)" 
	pack .entAddress .lblAddress -in .frmAddress -side left
	
	frame .frmData
	iwidgets::entryfield .entData -labeltext "Data_width" -labelpos w -textvariable Para(Width) \
