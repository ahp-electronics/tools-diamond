#$Header: L:/module/pll/sn5w00/gui/core_template.tcl 1.1 mghuang Exp $

#Rev 1.1: Diamond 3.5: Initial version
...
proc FrequencyDRC {} {
	...
	if {$Para(CLKOP_MUXA) == "DISABLED"} {
		...
		if {$Para(FEEDBK_PATH) == "CLKOP"} {
			if {$Para(CLKOP_FREQ) < "10"} {
				set msg "The Feedback frequency must be greater than 10 MHz.  (Ffbk = Fout / N). \
...
proc Create_Image {} {
	...
	switch $tcl_platform(platform) {
		windows {AttributeSetting 620 600}
...
#--- Frequency ---
	...
	set rx_type_lst {LVDS SUBLVDS SLVS MIPI LVTTL33 LVCMOS33 \
...
#	grid .cbFRACN_ENABLE .entFRACN_DIV -in $frmCLKFB
	...
	iwidgets::combobox .cboCLKOP_TOL   -labeltext "\nTolerance %" -labelpos n -width 10 -textvariable Para(CLKOP_TOL) -selectioncommand EditFreq -editable false
	...
	iwidgets::combobox .cboCLKOS_TOL   -labeltext "\nTolerance %" -labelpos n -width 10  -textvariable Para(CLKOS_TOL) -selectioncommand EditFreq -editable false
	...
	iwidgets::combobox .cboCLKOS2_TOL   -labeltext "\nTolerance %" -labelpos n -width 10  -textvariable Para(CLKOS2_TOL) -selectioncommand EditFreq -editable false
	...
	iwidgets::combobox .cboCLKOS3_TOL   -labeltext "\nTolerance %" -labelpos n -width 10  -textvariable Para(CLKOS3_TOL) -selectioncommand EditFreq -editable false
