#$Header: L:/module/pll/machxo/gui/core_template.tcl 1.5 mghuang Exp $

#Rev 1.5: Diamond 2.0 IPExpress: Fixed CR_57445, change CLKI & CLKOP lower bound frequency limit from 25 to 18.
...
proc ChangeValueCmd {} {
	...
	if {$Para(arch) == "ep5g00" || $Para(arch) == "mj5g00"}  {
		set max 420.0000
	} 
	if {$Para(IFrq) < 18.0000 || $Para(IFrq) > $max} {
		.entIFrq.lwchildsite.entry config -fg red
	} else  {
		.entIFrq.lwchildsite.entry config -fg black
	} 
	if {$Para(mode) == "normal"}  {
		if {$Para(U_OFrq) < 18.0000 || $Para(U_OFrq) > $max} {
		...
		if {$Para(Type) == "ehxpllb" || $Para(Type) == "ehxpllc"}  {
			if {$Para(U_KFrq) < 0.1953 || $Para(U_KFrq) > 210.0000} {
...
proc CheckMg5g00Para {}  {
	...
	set ret [IsInRange "CLKI Frequency" 18.0000 $max $Para(IFrq)]
	if {$ret == -1} {return -1}
	if {$Para(mode) == "normal"}  {
		set ret [IsInRange "CLKOP Frequency" 18.0000 $max $Para(U_OFrq)]
		if {$ret == -1} {return -1}

		if {$Para(Type) == "ehxpllb" || $Para(Type) == "ehxpllc"}  {
			set ret [IsInRange "CLKOK Frequency" 0.1953 210.0000 $Para(U_KFrq)]
			if {$ret == -1} {return -1}
		} 
	} 
...
proc GenerateCB {} {
	...
	#lappend str -phase_cntl DYNAMIC -pll_rank 0
...
proc Create_Image {} {
	...
	#--- Page #1 ---
		...
		radiobutton .rdoAdvanced -text "Divider Mode"   -command {EditFreq;CallDrawPins} -width 16 -variable Para(mode) -value advanced
	...
	#	iwidgets::combobox .cboFB_Mode -labeltext "Feedback\nMode" -labelpos n -width 10 -textvariable Para(fb_mode) -editable false -selectioncommand EditFreq
		...
		# 26615
		checkbutton .cbEnCLKOS -text "Enable CLKOS" -variable Para(EnCLKOS) -command {EditFreq;CallDrawPins}
