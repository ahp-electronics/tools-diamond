#$Header: L:/module/pll/latticexp2/gui/rcs/core_template.tcl 1.22 2009/10/10 09:27:22Z mghuang Exp $
#!/bin/sh
...
set frmClkOK [$win.frmRight1.frmClkOK childsite]
	...
	if {$Para(mode) == "advanced"}  {
		...
		if {[string is integer $n] == 0}  {
			...
			if {[string is integer $v] == 0}  {
				$frmClkOP.frmP1.cboVDiv.lwchildsite.entry config -fg red
				set v [SetDivider3 $v 0]
				set Para(Post) $v
			}
			set Para(BoolFlag) 0
		...
		$frmClkOP.frmP1.entOFrq.lwchildsite.entry config -fg black
	} 
	...
	if {$Para(mode) == "normal"}  {
		if {$Para(ClkOPBp) == 0} {
			...
			if {$ret == -1} {
				$frmClkOP.frmP1.entOFrq.lwchildsite.entry config -fg red
				set Para(BoolFlag) 0
				return -1
			} else {
				$frmClkOP.frmP1.entOFrq.lwchildsite.entry config -fg black
...
set frmClkOK [$win.frmRight1.frmClkOK childsite]
	...
	# PLL Type = ehxpllb
		...
		#Frequency Mode
			...
			$frmClkOP.frmP1.cboVDiv config -state disabled
			$frmClkOK.frame21.cboKDiv config -state disabled
			$frmClkOP.frmP1.entU_OFrq config -state normal -foreground black
			$frmClkOP.frmP1.cboOP_Tol config -state normal -foreground black
			...
			if {$Para(PhaseDuty) == "Dynamic"} {
				...
				if {$Para(DutyTrimP) == "Rising"} {
					...
					$frmTrimP.cboDelayMultP clear list
					$frmTrimP.cboDelayMultP insert list end 0 1 2 3 4 5 6 7
					if {[lsearch {0 1 2 3 4 5 6 7} $Para(DelayMultP)] == -1} {
						set Para(DelayMultP) 0
					}
				}
			} else {
			...
			if {$Para(ClkOPBp) == 1}  {
				$frmClkOP.frmP1.entU_OFrq config -state disabled
				$frmClkOP.frmP1.cboOP_Tol config -state disabled
				$frmTrimP.rdoTrimP0 config -state disable
				$frmTrimP.rdoTrimP1 config -state disable
				$frmTrimP.cboDelayMultP config -state disabled
			} 
		...
		#Divider Mode
			...
			$frmClkOP.frmP1.cboVDiv config -state normal
			$frmClkOK.frame21.cboKDiv config -state normal -foreground black
			$frmClkOP.frmP1.entU_OFrq config -state disabled -foreground gray50
			$frmClkOP.frmP1.cboOP_Tol config -state disabled -foreground gray50
	...
	if {$Para(EnCLKOS) == 0}  {
		...
		$frmDutyTrim.cboDelayMult config -state disabled
		...
		if {$Para(ClkOSBp) == 1}  {
			...
			if {$Para(PhaseDuty) == "Static"} {
				...
				$frmDutyTrim.cboDelayMult config -state normal
				if {$Para(DutyTrimS) == "Rising"} {
					$frmDutyTrim.cboDelayMult clear list
					$frmDutyTrim.cboDelayMult insert list end 0 1 2 3
				} else {
					$frmDutyTrim.cboDelayMult clear list
					$frmDutyTrim.cboDelayMult insert list end 0 1 2 3
					if {[lsearch {0 1 2 3} $Para(DelayMultS)] == -1} {
						set Para(DelayMultS) 0
					}
				}
			}
	...
	#ISPL_CR_36005  10 ¨C 435 MHz
	set max 435.0000
	if {$Para(IFrq) < 10.0000 || $Para(IFrq) > $max} {
		$frmClkI.entIFrq.lwchildsite.entry config -fg red
	} else  {
		$frmClkI.entIFrq.lwchildsite.entry config -fg black
	} 
	if {$Para(mode) == "normal"}  {
		if {$Para(U_OFrq) < 10.0000 || $Para(U_OFrq) > $max} {
			$frmClkOP.frmP1.entU_OFrq.lwchildsite.entry config -fg red
		} else  {
			$frmClkOP.frmP1.entU_OFrq.lwchildsite.entry config -fg black
		} 

		if {$Para(Type) == "ehxpllb" || $Para(Type) == "ehxpllc"}  {
			if {$Para(U_KFrq) < 0.078 || $Para(U_KFrq) > 217.5000} {
	...
	if {[string is integer $v] == 1}  {
		$frmClkOP.frmP1.cboVDiv.lwchildsite.entry config -fg black
	} 
...
proc CheckMg5g00Para {}  {
	...
	#ISPL_CR_36005  10 ¨C 435 MHz
	set max 435.0000
	set ret [IsInRange "CLKI Frequency" 10.0000 $max $Para(IFrq)]
	if {$ret == -1} {return -1}
	if {$Para(mode) == "normal"}  {
		set ret [IsInRange "CLKOP Frequency" 10.0000 $max $Para(U_OFrq)]
		if {$ret == -1} {return -1}

		if {$Para(Type) == "ehxpllb" || $Para(Type) == "ehxpllc"}  {
			set ret [IsInRange "CLKOK Frequency" 0.078 217.5000 $Para(U_KFrq)]
			if {$ret == -1} {return -1}
		} 
	} 
...
proc GenerateCB {} {
	...
	if {$Para(ClkOPBp) == 1}  {
		lappend str -bypassp;set u_ofrq $Para(IFrq)
	} else {
		if {$Para(PhaseDuty) == "Dynamic"} {
			if {$Para(DutyTrimP) == "Rising"} {
	...
	if {$Para(EnCLKOS) == 0}  {
		...
		if {$Para(PhaseDuty) == "Dynamic"} {
			if {$Para(DutyTrimS) == "Rising"} {
				...
				if {$Para(DelayMultS) != 0} {lappend str -trims [expr 1*$Para(DelayMultS)]}
			} else {
				#set tmp [expr -1*$Para(DelayMultS)]
				lappend str -trims [expr -1*$Para(DelayMultS)]
			}
	...
	if {$Para(PDPDP) == 1} {lappend str -pwd}
...
set win [.sf childsite]		
		...
		set frmClkOP [$win.frmRight1.frmClkOP childsite]
		checkbutton $frmClkOP.cbClkOPBp -text "Bypass (CLKOP=CLKI)" -variable Para(ClkOPBp) -command {ChangeClkOPCmd;EditFreq;CallDrawPins}
		pack $frmClkOP.cbClkOPBp -in $frmClkOP -anchor e -padx 12
		
		frame $frmClkOP.frmP1
		iwidgets::combobox   $frmClkOP.frmP1.cboVDiv   -labeltext "\nDivider"            -labelpos n -width 6  -textvariable Para(Post) -editable false -selectioncommand EditFreq
		iwidgets::entryfield $frmClkOP.frmP1.entU_OFrq -labeltext "Desired\nFrequency" -labelpos n -width 12 -textvariable Para(U_OFrq) -validate real
		iwidgets::combobox   $frmClkOP.frmP1.cboOP_Tol -labeltext "\nTolerance"          -labelpos n -width 8  -textvariable Para(OP_Tol) -editable false -selectioncommand EditFreq
		iwidgets::entryfield $frmClkOP.frmP1.entOFrq   -labeltext "Actual\nFrequency"    -labelpos n -width 12 -textvariable Para(OFrq) -fixed 10 -state disabled -disabledforeground black
		#ISPL_CR_29211
		$frmClkOP.frmP1.cboVDiv insert list end 2 4 8 16 32 48 64 80 96 112 128
		#24674
		$frmClkOP.frmP1.cboOP_Tol insert list end 0.0 0.1 0.2 0.5 1.0 2.0 5.0 10.0
		pack $frmClkOP.frmP1.cboVDiv $frmClkOP.frmP1.entU_OFrq $frmClkOP.frmP1.cboOP_Tol $frmClkOP.frmP1.entOFrq -in $frmClkOP.frmP1 -side left -padx 2 -fill x
		$frmClkOP.frmP1.entU_OFrq.lwchildsite.entry config -validate key -vcmd {
			after idle {EditFreq;CallDrawPins}
			return 1
		}
		...
		pack $frmClkOP.frmP1 $frmClkOP.frmTrimP -in $frmClkOP -fill x -anchor w
		...
		pack $frmStatic.cboPhase $frmStatic.cboDuty -in $frmStatic -side left -padx 4
		...
		iwidgets::combobox $frmDutyTrim.cboDelayMult -labeltext "Delay Multiplier" -width 6 -textvariable Para(DelayMultS) -editable false -selectioncommand EditFreq
		pack $frmDutyTrim.rdoDutyTrim0 $frmDutyTrim.rdoDutyTrim1 $frmDutyTrim.cboDelayMult -in $frmDutyTrim -side left -padx 2
		$frmDutyTrim.cboDelayMult insert list end 0 1 2 3
		...
		pack $frmClkOK.frame2.cbEnCLKOK -in $frmClkOK.frame2 -side left  -anchor w
		pack $frmClkOK.frame2.cbClkOKBp -in $frmClkOK.frame2 -side right -anchor e
		...
		checkbutton $win.frmLeft1.cbPDPDP -text "Provide Dynamic Power Down Port" -variable Para(PDPDP) -command {EditFreq;CallDrawPins}
		...
		pack $win.frmLeft1.cbPCDR -in $win.frmLeft1 -anchor w -pady 4
		pack $win.frmLeft1.cbPDPDP -in $win.frmLeft1 -anchor w -pady 4
		pack $win.frmRight1.frmClkOP $win.frmRight1.frmClkOS $win.frmRight1.frmClkOK -in $win.frmRight1 -fill x
