#$Header: L:/module/ram_dp_true/machxo2/gui/core_template.tcl 1.9 mghuang Exp $

#Rev 1.9: Diamond 3.1 IPExpress: Fixed SOF-118884
#Rev 1.8: Diamond 3.1 IPExpress: XO3 support
# Rev 1.7: Diamond 2.0.1 IPExpress: Fixed CR_57842
# Rev 1.6: Diamond 1.3 IPExpress: Fixed cr_52774, -writemodeA -writemodeB
# Rev 1.5: 8.1s1 IPExpress: Fixed #49829
# Rev 1.1: 8.1a IPExpress: Fixed cr_48133,48193
# Rev 1.2: UM Rev 3.11.
# Rev 1.3: Fixed #48134
# Rev 1.4: Fixed cr_48848
...
proc ChangeValueCmd {}  {
	...
	if {$Para(ROutputEn) == "1"} {
		.cbRClkEn config -state normal
	} else {
		.cbRClkEn config -state disabled
		set Para(RClockEn) 0
	}
	if {$Para(WOutputEn) == "1"} {
		.cbWClkEn config -state normal
	} else {
		.cbWClkEn config -state disabled
		set Para(WClockEn) 0
	}
	if {$Para(ROutputEn) == "1" || $Para(WOutputEn) == "1"} {
...
proc CheckParameterCB {} {
	...
	set ret [IsInRange "Address Depth of A Port" 2 65536 $Para(RAddress)]
	if {$ret == -1} {return -1}
	set ret [IsInRange "Data Width of A Port" 1 256 $Para(RData)]
	if {$ret == -1} {return -1}
	set ret [IsInRange "Address Depth of B Port" 2 65536 $Para(WAddress)]
	if {$ret == -1} {return -1}
	set ret [IsInRange "Data Width of B Port" 1 256 $Para(WData)]
	if {$ret == -1} {return -1}
	...
	set msg "Ratio of Data Width values (Width/Width) cannot be greater than 8."
	if {$x > 8}  {
		tk_messageBox -default ok -icon warning -message $msg -title $tit -type ok
		return -1
	} 
...
proc GenerateCB {} {
	...
	lappend str -aaddr_width [expr int(ceil(log($Para(RAddress))/log(2)))] -widtha $Para(RData)
	lappend str -baddr_width [expr int(ceil(log($Para(WAddress))/log(2)))] -widthb $Para(WData)
	lappend str -anum_words $Para(RAddress)
	lappend str -bnum_words $Para(WAddress)
	if {$Para(enByte) == "1"} {lappend str -byte $Para(ByteSize)}
	if {$Para(ROutputEn) == "1"} {lappend str -outdataA REGISTERED}  
	if {$Para(WOutputEn) == "1"} {lappend str -outdataB REGISTERED}  
	if {$Para(RClockEn) == "1"} {lappend str -out_enable_0}
	if {$Para(WClockEn) == "1"} {lappend str -out_enable_1}
...
proc Create_Image {} {
	...
	MainInterface
if {$Para(arch) == "se5c00" || $Para(arch) == "se5r00"} {
	AttributeSetting 460 580
} else  {
	AttributeSetting 460 550
}
	...
	#--- Page #1 ---
		...
		#*** Size of block RAM ***
		label .lblSize -text "Specify the size of the RAM_DP_TRUE" 
		
		iwidgets::Labeledframe .frmRead -labeltext "A Port" -labelpos nw
		...
		pack .entRAddress .lblRAddress .entRData .lblRData -in .frmRDataAdd -side left
		...
		#*** checkbutton ***
		frame .frmROutput
		checkbutton .cbROutputEn  -text "Enable Output Register"  -variable Para(ROutputEn) -command {ChangeValueCmd}
		checkbutton .cbRClkEn -text "Enable Output ClockEn" -variable Para(RClockEn) -command {ChangeValueCmd}
		pack .cbROutputEn .cbRClkEn -in .frmROutput -side left -padx 10
		
		pack .frmRDataAdd -in $frmRead
		pack .frmROutput -in $frmRead -anchor w -padx 45

		iwidgets::Labeledframe .frmWrite -labeltext "B Port" -labelpos nw
		...
		pack .entWAddress .lblWAddress .entWData .lblWData -in .frmWDataAdd -side left
		...
		#*** checkbutton ***
		frame .frmWOutput
		checkbutton .cbWOutputEn  -text "Enable Output Register"  -variable Para(WOutputEn) -command {ChangeValueCmd}
		checkbutton .cbWClkEn -text "Enable Output ClockEn" -variable Para(WClockEn) -command {ChangeValueCmd}
		pack .cbWOutputEn .cbWClkEn -in .frmWOutput -side left -padx 10

		pack .frmWDataAdd -in $frmWrite
		pack .frmWOutput -in $frmWrite -anchor w -padx 45
		...
		if {$Para(arch) == "se5c00" || $Para(arch) == "se5r00"} {
			frame .frmNoInit
			checkbutton .cbno_init -text "No initialization(power up with random data),\nfor PUF simulation,\nplease select the initialization for all 0's,all 1's or Memory file."  -justify left  -variable Para(no_init) -command {ChangeValueCmd}
...
if {$Para(arch) == "se5c00" || $Para(arch) == "se5r00"} {
	...
	pack .frmProperties -in $page -side top -fill both -expand 1
		...
		Create3RadioItem $frmAdvanced PortAWM "" 20 \
			...
			"Read before Write" ReadBeforeWrite 15 top Para(WriteA) ChangeValueCmd
		...
		Create3RadioItem $frmAdvanced PortBWM "" 20 \
			"Normal" Normal \
			"Write Through" WriteThrough \
			"Read before Write" ReadBeforeWrite 15 top Para(WriteB) ChangeValueCmd
