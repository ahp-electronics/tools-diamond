#$Header: L:/module/mmac/gui/core_template.tcl 1.8 mghuang Exp $

#Rev 1.8: Diamond 2.1 IPExpress: Fixed CR_52546 - Output register is always enabled for MAC/MMAC. to make it consistent to Application Handbook, UM not updated.
#Rev 1.7: Diamond 2.0 IPExpress: SCR_1128, UM rev 1.23, CR_54604
...
proc ECP3CmdLine {} {
	...
	set str "-arch $Para(arch) -type dspmmac"
	...
	switch $Para(addsub) {
	    "ACCUM+MULTA+MULTB" {lappend str -macadd}
	    "ACCUM+MULTA-MULTB" {lappend str -macsub}
	    "ACCUM+MULTA+/-MULTB" {lappend str -macaddsub}
	}
...
proc Create_Image {} {
	...
	iwidgets::labeledframe .basicsize -labeltext "Size of the DSPMMAC block" -labelpos nw
	set basicsize [.basicsize childsite]
	
	label .lblInputA -text "Input A0/A1"
	label .lblInputB -text "Input B0/B1"
	...
	iwidgets::combobox .cboSignedA -width 9 -editable false -textvariable Para(SignedA) -selectioncommand {ChangeValueCmd}
	.cboSignedA insert list end Signed Unsigned Dynamic
	iwidgets::combobox .cboSignedB -width 9 -editable false -textvariable Para(SignedB) -selectioncommand {ChangeValueCmd}
	.cboSignedB insert list end Signed Unsigned Dynamic
	
	iwidgets::combobox .cboInputA -width 9 -editable false -textvariable Para(InputA) -selectioncommand {ChangeValueCmd}
	.cboInputA insert list end Parallel Shift Dynamic
	iwidgets::combobox .cboInputB -width 9 -editable false -textvariable Para(InputB) -selectioncommand {ChangeValueCmd}
	...
	label .lblcbaddsub -text "Operation"
	iwidgets::combobox .cbocbaddsub -width 24 -editable false -textvariable Para(addsub) -selectioncommand {ChangeValueCmd}
	.cbocbaddsub insert list end ACCUM+MULTA+MULTB ACCUM+MULTA-MULTB ACCUM+MULTA+/-MULTB
	...
	grid .lblcbaddsub -row 5 -column 0 -in $basicsize -padx 2 -pady 2
	grid .cbocbaddsub -row 5 -column 1 -columnspan 2 -in $basicsize -padx 2
	...
	checkbutton .ckEnIRA -text "Enable Input\nRegister A0/A1" -variable Para(EnIRA) -command ChangeValueCmd
	checkbutton .ckEnIRB -text "Enable Input\nRegister B0/B1" -variable Para(EnIRB) -command ChangeValueCmd
	...
	#CR_52546 - Output register is always enabled for MAC/MMAC. to make it consistent to Application Handbook
	.ckEnOR config -state disabled
