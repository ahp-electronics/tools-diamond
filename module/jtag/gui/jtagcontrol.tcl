...
package provide JtagCore 1.0.0
...
proc Generate {} {
	...
	if {[string length $Para(ModuleName)] == 0} {
		...
		#		-message "Same name design files are already exists, overwrite or not?" \
		#		-type yesno -icon question -title "LPC file"]
		#	if {[string compare -nocase $answer "no"] == 0} {return}
		#}
		set ret [GenerateCB]
		if {$ret != -1} {
...
proc create_core_script {} {
    global editor editor_no Para
    ...
    puts $fid ""
    #puts $fid "#-------------------------------------------"
    #puts $fid "# User copies to project directory"
    #puts $fid "#-------------------------------------------"
    puts $fid "#!/usr/local/bin/wish"
    puts $fid ""
    #puts $fid "#-------------------------------------------"
    #puts $fid "# Can be dervied from environment variables"
    #puts $fid "#-------------------------------------------"
    puts $fid "set Para(ProjectPath) \"$Para(path)\""
    puts $fid "set Para(lib) \"$Para(lib)\""
    puts $fid "set Para(CoreName) \"$Para(CoreName)\""
    puts $fid "set Para(family) \"$Para(family)\""
    puts $fid "set Para(Family) \"$Para(Family)\""
    puts $fid "set Para(GenSpi) \"$Para(GenSpi)\""
    puts $fid "set Para(GenTracy) \"$Para(GenTracy)\""
    puts $fid "set Para(GenER1) \"$Para(GenER1)\""
    puts $fid "set Para(Synpro) \"$Para(Synpro)\""
    puts $fid "set Para(design) \"$Para(design)\""
    set Para(family)
    puts $fid ""
    #puts $fid "#-----------------------------------"
    #puts $fid "# Must be set by customer"
    #puts $fid "#-----------------------------------"
    puts $fid "set Para(ModuleName) \"$Para(module)\""
    puts $fid ""
    puts $fid "lappend auto_path \"$Para(CoreDir)\""
    puts $fid ""
    puts $fid "lappend auto_path $Para(scriptfile)"
    puts $fid "package require Core_Generate"
    puts $fid ""
    puts $fid "lappend auto_path \"$Para(runproc_path)\""
    puts $fid "package require runcmd"
    puts $fid ""
    puts $fid "set Para(install_dir) \"$Para(leverpath)\""
    puts $fid ""
    puts $fid "GenerateCore"
...
proc run_script {} {
    ...
    set Para(gen_script) [file join $Para(ProjectPath) generate_core.tcl]
...
proc Eval {command} {
    ...
    #} else {
    #    set Para(event) [EvalSlave history nextid]
...
#tk_messageBox -default ok -icon info -message "Getting pin_side_list= $pin_side_list"  \
#	-title "Info" -type ok
	...
	set right_pin_num [expr [lindex $pin_side_list 1]-1]
        ...
        set total_pin_num [expr $left_pin_num + $right_pin_num+2]

        set half_pin_num [expr $total_pin_num/2]
	...
	set left_bus_width [lrange $bus_width_list 0 $left_pin_num]
        if {$left_pin_num +1 < $half_pin_num } {
	   set incr_y [expr $rect(height)/[expr $left_pin_num+2]]
        } else {
           set incr_y [expr $rect(height)/[expr $total_pin_num+1]]
        }
	set y [expr $rect(y)+$incr_y]

#tk_messageBox -default ok -icon info -message "Setting l_pin_position $l_pin_position"  \
#	-title "Info" -type ok

        set idx 0

#tk_messageBox -default ok -icon info -message "Setting parray $parray"  \
#	-title "Info" -type ok
	...
	foreach pin_name $left_pin_name arrow_direction $left_pin_direction \
		...
		set y [expr {$y+$incr_y}]
                ...
                set idx [expr $idx +1]
                set oidx [expr $left_pin_num +1]
                set oidx [expr $oidx/2]

                if {$left_pin_num +1 >= $half_pin_num &&
                    $idx == $oidx } {
                   set y [DrawRightPins $diagram $W $y $incr_y $pin_name_list $pin_direction_list $bus_width_list $pin_side_list]
                }
	}
	   ...
	   set incr_y [expr $rect(height)/[expr $right_pin_num+2]]
 	   set y [expr $rect(y) + $incr_y]
           DrawRightPins $diagram $W $y $incr_y $pin_name_list $pin_direction_list $bus_width_list $pin_side_list]
        }

#tk_messageBox -default ok -icon info -message "Setting l_pin_position $l_pin_position"  \
#	-title "Info" -type ok

        set offsety [font metrics $ft -linespace]
#tk_messageBox -default ok -icon info -message "Setting offsety= $offsety"  \
#	-title "Info" -type ok
...
proc DrawRightPins {diagram W y incr_y pin_name_list pin_direction_list bus_width_list pin_side_list} {
	global Para ft
...
#tk_messageBox -default ok -icon info -message "Setting output y= $y"  \
#	-title "Info" -type ok

                #set lpin_len [llength $l_pin_position]
                #set yy0 $y0
