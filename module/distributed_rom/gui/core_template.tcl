#$Header: L:/module/distributed_rom/gui/core_template.tcl 1.11 mghuang Exp $

#Rev 1.11: Diamond 2.2 IPExpress: Sapphire SCR_1014
#Rev 1.10: Diamond 2.2 IPExpress: Fixed cr_59051, Sapphire UM rev 2.0.
...
proc ChangeValueCmd {} {
	...
	if {$Para(Addresses) >= 2 && $Para(Addresses) <= 256}  {
		set data_width 128
	} 
	if {$Para(Addresses) >= 257 && $Para(Addresses) <= 2048}  {
		set data_width 64
	} 
	if {$Para(Addresses) >= 2049 && $Para(Addresses) <= 8192}  {
		set data_width 2
	} 
	if {$Para(Addresses) > 8192}  {
		set data_width 2
	} 
...
proc CheckOrca5Para {}  {
	...
	set ret [IsInRange "Addresses" $add_width $MaxWidth $Para(Addresses)]
	if {$ret == -1} {return -1}
	
	if {$Para(arch) == "sa5p00"} {
		set temp [expr int(ceil(log($Para(Addresses))/log(2)))]
		if {[expr pow(2,$temp) - $Para(Addresses)] > 0}  {
			set msg "Address depth width only powers of 2 are supported."
	...
	if {$Para(Addresses) >= $add_width && $Para(Addresses) <= 256}  {
		set data_width 128
	} 
	if {$Para(Addresses) >= 257 && $Para(Addresses) <= 2048}  {
		set data_width 64
	...
	if {$Para(Addresses) >= 2049 && $Para(Addresses) <= 8192}  {
		set data_width $add_width
	} 
	set ret [IsInRange "Data" 1 $data_width $Para(Data)]
	if {$ret == -1} {return -1}
...
proc Orca5CmdLine {} {
	global Para
	set str "-arch $Para(arch) -type rom -num_rows $Para(Addresses) -data_width $Para(Data)"
	if {$Para(LUT) == "1"} {lappend str -pipe_final_output -enable}
	#ISPL_CR_30146 
	if {$Para(LUT) == "0"} {lappend str -async}

	if [string length $Para(MemFile)] {lappend str -memfile $Para(MemFile) -memformat $Para(MemFormat)}
	return $str
} 

# *** command-line for LATTICE-XP ***
proc XPECCmdLine {} {
	global Para
	set Para(Addresses) [string trimleft $Para(Addresses) 0]
	set str "-arch $Para(arch) -dram -type romblk -addr_width [expr int(ceil(log($Para(Addresses))/log(2)))] -num_words $Para(Addresses) -data_width $Para(Data)"
	if {$Para(LUT) == "1"} {
...
# *** command-line for or3,4 & FPSC device family. ***
proc GeneralCmdLine {}  {
	global Para
	set str "-arch $Para(arch) -type rom -num_rows $Para(Addresses) -data_width $Para(Data)"
	if $Para(LUT) {append str " -mem_mux"}
	if $Para(PI) {
		if $Para(RA) {append str " -pipe_read_addr_rep"
		} else {append str " -pipe_read_addr"}
	}
	if $Para(RFO) {append str " -pipe_final_output"}
	if [string length $Para(MemFile)] {append str " -memfile \"$Para(MemFile)\""}
	return $str
} 
