#$Header: L:/module/multaddsub_a/gui/rcs/core_template.tcl 1.16 2009/11/10 06:47:48Z mghuang Exp $
#!/bin/sh
...
proc CheckParameterCB {} {
	...
	set ret [IsInRange "Input A0/A1 Width" 2 36 $Para(pmi_dataa_width)]
	if {$ret == "-1"} {return -1}
	set ret [IsInRange "Input B0/B1 Width" 2 36 $Para(pmi_datab_width)]
	if {$ret == "-1"} {return -1}
...
proc GenerateCB {} {
	...
	set str "-arch $Para(arch) -type dspmadd -simple_portname"
        set l_has_reg 0

	if {$Para(pmi_implemenntation) == "LUT"}  {lappend str -pfu_mult -pfu_add}
...
proc Create_Image {} {
	...
	frame .frmWidth1
	CreateLabelEntryLabel .frmWidth1 WidthA "Input A0/A1 Width" 4 "(2 - 36)" "" Para(pmi_dataa_width) integer
	...
	frame .frmWidth2
	CreateLabelEntryLabel .frmWidth2 WidthB "Input B0/B1 Width" 4 "(2 - 36)" "" Para(pmi_datab_width) integer
	...
	CreateLabelEntryLabel $frmP1 WidthP "Sum Width" 4 "" "" Para(pmi_datap_width) integer
	
	pack .frmWidth1 .frmWidth2 .frmWidthP -in $frmP1 -side top -anchor w -pady 4
	
	.entryWidthP config -state disable
	.lblWidthA config -width 18 -anchor w
	.lblSignA config -width 14 -anchor w
	.lblWidthB config -width 18 -anchor w
	.lblSignB config -width 14 -anchor w
	.lblWidthP config -width 18 -anchor w
	...
	#CenterPosition
	GeneralConfig
#	set Para(topic) "multaddsub_arithmetic.htm"
