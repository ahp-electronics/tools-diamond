#$Header: L:/module/mac_a/gui/rcs/core_template.tcl 1.17 2009/11/10 06:47:47Z mghuang Exp $
#!/bin/sh
...
proc CallDrawPins {}  {
	...
	if {[string length $Para(pmi_dataa_width)] > 0 &&
		...
		[string length $Para(pmi_accum_width)] > 0 } {DrawPins}
} 
...
proc ChangeValueCmd {} {
	...
	set Para(pmi_datap_width) [expr $Para(pmi_dataa_width)+$Para(pmi_datab_width)+$Para(pmi_accum_width)+0]
...
proc CheckParameterCB {} {
	...
	set ret [IsInRange "A Width" 2 36 $Para(pmi_dataa_width)]
	if {$ret == "-1"} {return -1}
	set ret [IsInRange "B Width" 2 36 $Para(pmi_datab_width)]
	if {$ret == "-1"} {return -1}

	set ret [IsInRange "Accumulator Width" 1 32 $Para(pmi_accum_width)]
	if {$ret == "-1"} {return -1}
...
proc GenerateCB {} {
	...
	set str "-arch $Para(arch) -type dspmac -simple_portname -clk0 -ce0 -rst0 "
	if {$Para(pmi_implemenntation) == "LUT"}  {lappend str -pfu_mult -pfu_accum}

	#ISPL_CR_30885
	lappend str -m$Para(pmi_operation)
	#ISPL_CR_30839
	lappend str -widtha $Para(pmi_dataa_width) -widthb $Para(pmi_datab_width) -widthaccum $Para(pmi_datap_width)
	...
	if {$Para(pmi_input_reg) == "Yes"}  {lappend str -input_reg} 
	if {$Para(pmi_output_reg) == "Yes"}  {lappend str -output_reg} 
...
proc Create_Image {} {
	...
	CreateRadioItem $frmP1 Operation "Add/Sub Operation" 8 Add add Sub sub 6 left Para(pmi_operation) {ChangeValueCmd}
	.lblOperation config -width 20 -anchor w
	pack .frmOperation -in $frmP1 -side top -anchor w -padx 10

	frame .frmWidth1
	CreateLabelEntryLabel .frmWidth1 WidthA "Input A Width" 5 "(2 - 36)" "" Para(pmi_dataa_width) integer
	...
	frame .frmWidth2
	CreateLabelEntryLabel .frmWidth2 WidthB "Input B Width" 5 "(2 - 36)" "" Para(pmi_datab_width) integer
	...
	pack .frmWidth1 .frmWidth2 -in $frmP1 -side top -anchor w -pady 4
	
	.lblWidthA config -width 14 -anchor w
	.lblSignA config -width 14 -anchor w
	.lblWidthB config -width 14 -anchor w
	...
	frame .frmWidth3
	#ISPL_CR_30431 
	CreateLabelEntry .frmWidth3 WidthM "Accumulator Width" "(1 - 32)" 5 e top Para(pmi_accum_width) integer
	CreateLabelEntry .frmWidth3 WidthP "Sum Output Bit Width" " " 4 e top Para(pmi_datap_width) integer
	.entryWidthP config -state disable -foreground gray50
	pack .frmWidth3 -in $frmP1 -side top -anchor w -padx 20 -pady 4

 	.entryWidthM.lwchildsite.entry config -validate key -vcmd {
			after idle {ChangeValueCmd}
			return 1
	}
	...
	#CenterPosition
	GeneralConfig
#	set Para(topic) "mac_arithmetic.htm"
