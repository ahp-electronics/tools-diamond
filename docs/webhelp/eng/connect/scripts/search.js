...
Search.InBrowser_Object = function () {
  ...
  stop_words_array = 'a about after all also am an and another any are as at be because been before being between both but by came can come copyright corporation could did do does each etc for from get goes got had has have he her here him himself his how if in into is it its lattice let like make many me might more most much must my never nor not now of off on one only or other our out over own said same see semiconductor set shall she should since so some still such take than that the their them then there these they this those though through to too under us use very was way we well were what when where which while who why will with would yes yet you your'.split(' ');
  this.stop_words = {};
        ...
        ajax.onreadystatechange = function () {
          ...
          if (ajax.readyState === 4) {
            ...
            if (window.JSON && window.JSON.parse) {
              info_data = window.JSON.parse(info_as_text);
            } else {
              info_data = eval('(' + info_as_text + ')');
            }
