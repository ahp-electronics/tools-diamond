...
// --------------------------------------------------------------------
//
// PMI Simulation File for Single Port Block RAM with Byte Enable
//
...
pmi_byte_size                  8|9                                          9
pmi_family  "XP2"|"SC"|"SCM"|"ECP2"|"ECP2M"|"ECP3"|"ECP5U"|"XO2"         "ECP2"
------------------------------------------------------------------------------
...
*/
//
// fpga\verilog\pkg\versclibs\data\pmi\pmi_ram_dq_be.v 1.4 20-FEB-2014 15:17:04 FGAO2
...
//Define internal Signals
   ...
   reg 				Wr_reg = 0;
   reg [((pmi_data_width+pmi_byte_size-1)/pmi_byte_size-1):0] ByteEn_reg = 0;

   reg [(pmi_data_width-1):0]   DataIn_reg_async = 0;
   reg [(pmi_addr_width-1):0] 	Address_reg_async = 0;
   reg 				Wr_reg_async = 0;
   reg [((pmi_data_width+pmi_byte_size-1)/pmi_byte_size-1):0] ByteEn_reg_async = 0;

   reg [(pmi_data_width-1):0]   DataIn_reg_sync = 0;
   reg [(pmi_addr_width-1):0] 	Address_reg_sync = 0;
   reg 				Wr_reg_sync = 0;
   reg [((pmi_data_width+pmi_byte_size-1)/pmi_byte_size-1):0] ByteEn_reg_sync = 0;
...
//Read-before-Write is supported for memory configurations in x9, x18, x36 datawidth. Data width should be set to match this requirement.
   ...
   initial begin
      if ((pmi_family != "SC") && (pmi_family != "SCM") && (pmi_family != "XO") &&
          (pmi_family != "LPTM") && (pmi_write_mode == "readbeforewrite"))
	begin
...
initial begin
	if (pmi_data_width <= pmi_byte_size) begin
		$display("\nError! Byte Enable feature is not supported for single-byte data widths!");
		$stop;
	end
end
...
//Choice between async/sync resetmode
   ...
   always @(DataIn_reg_sync, Address_reg_sync, Wr_reg_sync, ByteEn_reg_sync, ByteEn_reg_async,
	         DataIn_reg_async, Address_reg_async, Wr_reg_async, Q_reg_async, Q_reg_sync) begin
      if (pmi_resetmode == "async")
	begin
	   DataIn_reg = DataIn_reg_async;
	   Address_reg = Address_reg_async;
	   Wr_reg = Wr_reg_async;
		ByteEn_reg = ByteEn_reg_async;
	   Q_reg = Q_reg_async;
	end
      else
	begin
...
//Write Operation
   ...
   always @(Wr_reg, Address_reg,  DataIn_reg, ByteEn_reg, Clock_valid)
     begin
	memchg = ~memchg;
...
//	     mem[Address_reg] = DataIn_reg;
	     for (n = 0; n < pmi_data_width;n=n+1)
				
				if (ByteEn_reg[n / pmi_byte_size] == 1'b1)begin
					//mem[Address_reg * pmi_data_width + n] = DataIn_reg[n];
					mem[Address_reg][n] = DataIn_reg[n];
					end
	  end // if (Wr_reg)
...
//   always @(!Wr_reg, Address_reg, posedge Reset_sig, memchg, Clock_valid, mem[Address_reg])
   always @(posedge Reset_sig, memchg)
   begin
